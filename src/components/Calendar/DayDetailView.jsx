    ...
    const fetchLogs = async () => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                const uniqueNames = [...new Set(data.map(l => l.exercise))];
                const { data: exData } = await supabase
                    .from('exercises')
                    .select('name, equipment')
                    .in('name', uniqueNames);
                setEquipmentMap(Object.fromEntries((exData || []).map(ex => [ex.name, ex.equipment])));
            } else {
                setEquipmentMap({});
            }
                            ...
                            <div key={log.id} className={`bg-slate-900/40 border border-white/5 p-6 rounded-[1.5rem] relative overflow-hidden ${isMobile ? 'mb-3' : 'mb-4'}`}>
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        ...
                                        {equipmentMap[log.exercise] && <span className="text-[10px] text-slate-400 font-bold ml-1.5 align-middle">{equipmentMap[log.exercise]}</span>}
