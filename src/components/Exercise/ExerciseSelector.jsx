...
import { getExerciseGif, BODY_PART_I18N, setGlobalExerciseCache } from '../../utils/exerciseUtils';
import { useWindowSize } from '../../hooks/useWindowSize';

const GifRenderer = ({ nameEn, exerciseId, className = "w-full h-full object-cover", onClick, dataset = [] }) => {
    const gifUrl = getExerciseGif(nameEn, exerciseId, dataset);
...
const ExerciseSelector = ({ selection, setSelection, onExerciseSelect }) => {
    ...
    const handleExerciseClick = (ex) => {
        setSelection({ ...selection, exercise: ex, manualName: '' });
        if (onExerciseSelect) onExerciseSelect(ex.name);
    };

    const handlePreviewOpen = (e, ex) => {
        e.stopPropagation();
        const url = getExerciseGif(null, ex.id, exerciseDataset);
    ...
    const handleBack = () => {
        if (selection.exercise) setSelection({ ...selection, exercise: null });
        else if (selection.equipment) setSelection({ ...selection, equipment: null });
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
                ...
                <p className="text-slate-500 text-xs font-bold animate-pulse uppercase tracking-widest">{t('common.loading')}</p>
            </div>
        );
    }
        ...
        <div className="space-y-8">
            <div className="flex items-center gap-3">
                {(selection.part || selection.equipment || selection.exercise) && (
                    <button onClick={handleBack} className="p-2 bg-slate-800 text-slate-400 hover:text-white rounded-xl transition-colors">
            ...
            {!selection.part && (
                <div className="animate-fade-in">
                    ...
                    <div className="grid grid-cols-3 gap-4 w-full">
                        {BODY_PARTS.map(p => (
                            <button key={p.key} onClick={() => handlePartClick(p.key)} className="rounded-2xl font-black text-sm tracking-tighter transition-all duration-300 bg-slate-800/50 text-slate-400 hover:bg-slate-800 hover:text-slate-200 border border-slate-700/50 px-6 py-4 w-full">
                ...
                <div className="animate-fade-in">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                        {availableEquipments.map(eq => (
                            <button key={eq} onClick={() => handleEquipmentClick(eq)} className={`rounded-2xl font-black text-sm tracking-tighter transition-all duration-300 bg-slate-800/50 text-slate-400 hover:bg-slate-800 hover:text-slate-200 border border-slate-700/50 ${isMobile ? 'py-4' : 'py-3'}`}>
                                ...
                                {filteredExercises.map((ex) => (
                                    <div key={ex.id} onClick={() => handleExerciseClick(ex)} className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border ${selection.exercise?.id === ex.id ? 'bg-blue-600/20 border-blue-500' : 'bg-slate-800/30 border-white/5 hover:border-slate-600'}`}>
                                        ...
                                        <div className="shrink-0 flex items-center gap-1.5">
                                            <button onClick={(e) => { e.stopPropagation(); handleExerciseClick(ex); }} className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${selection.exercise?.id === ex.id ? 'bg-blue-500 text-white' : 'bg-slate-800 text-slate-500 hover:text-white'}`}>
                                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 6v6m0 0v6m0-6h6m-6 0H6" /></svg>
                                            </button>
                                        </div>
                                    </div>
                                ))}
                                <div onClick={handleDirectInputOpen} className="col-span-full flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-all border bg-slate-800/10 border-dashed border-slate-700 hover:border-slate-500 mt-2 group">
                                    <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-slate-900 shrink-0 border border-white/5"><svg className="w-5 h-5 text-slate-600 group-hover:text-blue-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg></div>
                                    <div className="flex-1"><p className="text-sm font-black italic text-slate-400 group-hover:text-white uppercase tracking-tighter">{t('exercise.noExerciseFound')}</p><p className="text-xs font-bold text-slate-600 uppercase tracking-widest">{t('exercise.customInputLink')}</p></div>
                                </div>
                            </>
                        )}
                    </div>
